...
export function shoot(xBounds: Bound, yBounds: Bound): number | null {
  let vx = 0;
  let bestY = -Infinity;

  while (isInRange(getFinalXPosition(vx), [0, xBounds[1]])) {
    const finalX = getFinalXPosition(vx);

    if (!isInRange(finalX, xBounds)) {
      vx++;
      continue;
    }
    ...
    while (vy > yBounds[0]) {
      ...
      while (!isGonePast(coord, velocity, xBounds, yBounds)) {
        ...
        if (isCoordInRange(coord, xBounds, yBounds)) {
          bestY = Math.max(currentBestY, bestY);
        }
      }
