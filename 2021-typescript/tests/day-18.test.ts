import { readInputFile, readLines } from "../src/utils";
import { Tree } from "../src/day-18/Tree";
import { RawSnailfishNumber, TreeNode } from "../src/day-18/types";
  ...
  test("Part 2", async () => {
    ...
    for (let i = 0; i < input.length; i++) {
      for (let j = 0; j < input.length; j++) {
        if (i !== j) {
          const tree = new Tree(input[i]);
          tree.mergeReduce([input[j]]);
